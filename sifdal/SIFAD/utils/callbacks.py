...
class LossHistory():
    ...
    def append_loss(self, epoch, loss, val_loss, accr_loss):
        ...
        with open(os.path.join(self.log_dir, "epoch_val_loss.txt"), 'a') as f:
            ...
            f.write("\n")
        with open(os.path.join(self.log_dir, "epoch_accr_loss.txt"), 'a') as f:
            f.write(str(accr_loss))
            f.write("\n")

        self.writer.add_scalar('loss', loss, epoch)
        self.writer.add_scalar('val_loss', val_loss, epoch)
        self.writer.add_scalar('accr_loss', accr_loss, epoch)
        self.loss_plot()
    ...
    def loss_plot(self):
        ...
        plt.plot(iters, self.accr_loss, 'blue', linewidth=2, label='accr loss')
        try:
...
class EvalCallback():
    ...
    def get_map_txt(self, image_id, image, class_names, map_out_path):
        ...
        with torch.no_grad():
            ...
            #   将图像输入网络当中进行预测！
            # ---------------------------------------------------------#
            outputs, rcho, scale_logits = self.net(images)
